...
// NOTA IMPORTANTE: Para la estabilidad en EasyPanel, asegúrate de que el contenedor de NOCTUS
// esté configurado para usar el puerto 8080 o de modificar esto a 8081 si es necesario.
const CREDENTIALS_PATH = '/workspace/credentials.json';
const SPREADSHEET_ID = '1jv-wydSjH84MLUtj-zRvHsxUlpEiqe5AlkTkr6K2248'; // ID CORREGIDO!
...
const RANGO_PRECIOS = 'A1:M999'; // ¡RANGO CORREGIDO! Precios promedios
...
// Ahora la función acepta el nombre de la hoja (sheetName) además del rango
...
app.get('/', (req, res) => {
    ...
    const endpoints = [
        // Rutas para la API Esclava (Miguelacho)
...
// 3. SERVICIO DE CONVERSIÓN CENTRALIZADO (MANTENEMOS LA RUTA ORIGINAL PARA COMPATIBILIDAD)
app.get('/convertir', async (req, res) => {
    ...
    try {
        ...
        // La matriz de Sheets usa cabeceras como 'USD_O', 'VES_O', y la primera columna es la clave.
        const claveMatrizDestino = `${D}_D`; // Fila a buscar (ej: USD_D)
        const claveMatrizOrigen = claveMatrizDestino in matrizGanancia[0] ? `${O}_D` : `${O}_O`; // Columna a buscar (ej: USD_O)
        
        // Buscamos la fila cuyo primer valor (la primera clave después de los headers) coincida con la moneda destino
        ...
        if (!filaDestino || !filaDestino[claveMatrizOrigen]) {
            // Buscamos la clave dinámica, si no existe usamos un fallback al header de origen
...
// --- MANEJADOR DE APAGADO ELEGANTE (Añadido para estabilidad) ---
process.on('SIGTERM', () => {
